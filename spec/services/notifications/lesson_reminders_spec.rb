...
RSpec.describe "Attendance-aware WhatsApp lesson reminders", type: :request do
  ...
  it "uses the selected numbers and exact Meta template parameter order" do
    ...
    expected_body = ["August 08, 2026 12:15 PM", lesson.teacher_profile.display_name,
  ...
  context "with recipient-local time zones", :aggregate_failures do
    ...
    it "keeps the absolute trigger instant while formatting a Cairo student's time instead of academy Riyadh" do
      ...
      expect(body_texts(delivery).first).to end_with("11:15 AM")
    ...
    it "uses the student timezone when delivery falls back to a guardian number" do
      ...
      expect(body_texts(delivery).first).to end_with("11:15 صباحًا")
