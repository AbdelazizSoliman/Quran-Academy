class EnrollmentLessonSchedule < ApplicationRecord
  ...
  attr_readonly :public_id, :enrollment_id
  ...
  validates :enrollment_id, uniqueness: { conditions: -> { where(status: "active") } }, if: :active?
  validate :date_order
  ...
  def enrollment_is_individual
    return unless enrollment&.student_profile&.session_type == "group"
