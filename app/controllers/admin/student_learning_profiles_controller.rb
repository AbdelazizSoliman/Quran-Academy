  ...
  class StudentLearningProfilesController < BaseController
    ...
    def update_section
      ...
      if section.errors.empty?
        redirect_to admin_student_learning_profile_path(@student, section: section_key),
    ...
    def set_student
      @student = StudentProfile.includes(:user, :assigned_teacher_profile).find(params.expect(:student_id))
    end
    ...
    def load_workspace
      ...
      @summary_items = if @profile
                         @profile.items.where(visibility: %w[internal guardian_safe student_safe])
    ...
    def persist_items(profile, section_key, section_record)
      ...
      values.each do |field_key, raw|
        value = normalize_value(section_key, field_key, raw)
        next if value.blank? && value != false && value != 0

        item = StudentLearningProfiles::UpsertItem.new(actor: current_user,
                                                       section: profile.sections.find_by!(section_key:),
                                                       field_key:, attributes: { value: }).call
        section_record.errors.add(:base, item.errors.full_messages.to_sentence) if item.errors.any?
      end
    end

    def normalize_value(section_key, field_key, raw)
      type = StudentLearningProfileFieldRegistry.fetch(section_key, field_key).fetch(:value_type)
      return raw.to_s.split(/\s*,\s*|\n/).compact_blank if type == "list"
      return %w[1 true].include?(raw) if type == "boolean"
    ...
    def profile_completion
      ...
      { started:, total: StudentLearningProfileSectionRegistry.keys.length,
    ...
    def section_params
      params.fetch(:section, {}).permit(:completion_state, :review_state)
    end
  end
